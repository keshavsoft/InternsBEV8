...
const StartFunc = ({ inRequestBody }) => {
    ...
    const LocalDefaultObject = LocalFuncBuildObject();
...
const LocalFuncBuildObject = () => {
    const result = {};
    const LocalColumns = ParamsJson.ColumnsWithSchema;

    LocalColumns.forEach(col => {
        let value;

        if ("defaultvalue" in col) value = col.defaultvalue;
        else if (col.type === "text") value = "";
        else if (col.type === "number") value = 0;
        else if (col.type === "file") value = null;
        else value = null;
